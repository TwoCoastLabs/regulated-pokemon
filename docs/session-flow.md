...
Calls #1 and #3 also admit, when `suggest` is on, a **suggest entry** — `{"kind": "suggest", "asks": [up to three short questions]}` — lifted out at decode and never a claim: the driver drops any that state a value (a digit, a certified id), the rest ride in the manifest's `suggestions` and render as the certified page's one labelled register (`data-unit="suggestions"`, items marked `data-suggestion`), which the walker attributes to the model, the affidavit swears visible, and the verifier holds to the record by equality. The kernel refuses a manifest whose suggestion states a value, so the register can carry a question and never a fact. *Since 2026-09-20 ([suggestions.md](suggestions.md)) the driver also drops any suggestion the records could not answer — read as the ask would be, through the dictionary's aliases with "it" as the answer's subject, or through the lesson door's matcher — and fills the register from the pack's own `presentation.nextAsks` table (a lesson's next lessons, a pair's other fields, a set's rankings, a subject's other fields), never repeating a text shown or asked earlier in the session; `suggest/gated` and `suggest/supplied` on the trail say what was dropped and what was added. No model call is added.*
...
5. **The contract.** Answer what was asked and assert nothing unrequested
   ...
   (A sentence making a question that names no certified subject a
   lesson in both scope branches was tried on 2026-09-21 and withdrawn
   at the bank gate, findings §38: against the same-host leg without it,
   it cost the strong model 19 of 411 samples and the weak 14, to lessons
   taught where a ranking, a recommendation or an honest pass was owed.
   The established-scope branch stays as it is;
   the ask that names nothing is answered by the carry-back naming the
   covering lesson instead.)
