...
per claim and must replay that way; lesson coverage in `v4.json`; the
compare table — comparisons over one pair as one table of both values, the
gap and the leader — in `v5.json`), and the replay sweep resolves packs by
